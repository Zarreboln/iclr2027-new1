...
import os,sys; sys.path.insert(0,os.path.dirname(os.path.abspath(__file__))); import house_style as H
H.rc(10); GRID,INK,MUTE=H.GRID,H.INK,H.MUTE; C=H.COMPONENTS
...
for i,d in enumerate(D): ax.text(x[i]+1.5*bw,d[4]+1.2,"%.1f"%d[4],ha="center",va="bottom",fontsize=7.5,color=INK)
...
ax.axvline(7.5,color=MUTE,lw=0.8,ls="--"); ax.text(3.5,103,"standard VPR benchmarks",ha="center",fontsize=9,color=H.FRAME); ax.text(12.5,103,"cross-environment VPR benchmarks",ha="center",fontsize=9,color=MUTE)
