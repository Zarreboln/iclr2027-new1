...
H.rc(9); GRID,INK,MUTE=H.GRID,H.INK,H.MUTE; C_UNW,C_W=H.MUTE,H.W_DARK
...
for k,(ds,name) in enumerate(ORDER):
    ...
    a.plot(x,100*c0,lw=1.6,color=C_UNW); a.plot(x,100*c1,lw=1.6,color=C_W); a.fill_between(x,100*c1,100*c0,where=(c0>=c1),color=H.W_LIGHT,alpha=.7,lw=0)
...
def draw_gallery(items,fname):
    ...
    for r,(ds,name) in enumerate(items):
        ...
        for col,(c,sel) in enumerate([(cu,su),(cw,sw)]):
            ...
            ax.add_patch(plt.Rectangle((0,0),RES,RES,fill=False,ec=H.Q_INK,lw=1.0)); ax.add_patch(plt.Rectangle((RES+GAP,0),RES,RES,fill=False,ec=H.D_INK,lw=1.0))
            for i in sel:
                if c[i]<=0: continue
