...
\par\smallskip{\scriptsize\emph{Notes.} The one thing we take from each baseline is its input resolution, which we set to that method's own default so that no baseline is handicapped by ours. \emph{Positives} is the mean number of correct database images per query, which sets how hard R@1 is on that benchmark. The radius is not uniform by design---each dataset keeps its own community convention, since $25$\,m indoors would mark a whole floor correct. Three choices deserve to be stated plainly because published numbers split on them: Nordland's $\pm10$-frame ground truth is the \emph{lenient} one; Tokyo uses the $315$-query subset and MSLS the $740$-query subset, and roughly half the literature uses the alternatives ($1125$ and $11$k); and VP-Air's database includes AnyLoc's $10{,}000$ distractor images, without which recall is substantially higher. Re-ranking uses a pool of $P{=}\min(1000,|\mathcal{D}|)$ throughout. $^\ast$SF-XL val is the design split: every constant in this paper is chosen on it, and it appears in no results table.}
\end{table}
