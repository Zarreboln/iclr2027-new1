...
\subsection{Trained methods on the standard benchmarks}
\label{app:trained-urban}
The trained block of Table~\ref{tab:breadth}, kept here for reference: the same eight benchmarks, the same protocol, methods that learn an aggregation layer from place labels.

\begin{table}[H]
\caption{\textbf{Trained or fine-tuned methods on the standard benchmarks.} R@1, same splits and ground truth as Table~\ref{tab:breadth}; \emph{Dim} is the first-stage descriptor dimensionality as evaluated. $^{\dagger}$after the method's own re-ranker; $^{\ddag}$without the cross-image encoder. Our two rows are repeated at the foot for comparison.}
\label{tab:trained-urban}
\centering
...
\midrule
\midrule
\multicolumn{11}{l}{Ours \emph{--- training-free}} \\
Ours\,\emph{v2}$^{\dagger}$ & 2048 & 95.66 & 92.49 & 97.46 & 89.05 & 83.03 & 99.11 & 98.95 & 68.07 & 90.48 \\
Ours\,\emph{v3}$^{\dagger}$ & 2048 & 96.12 & 93.74 & 98.41 & 89.32 & 96.56 & 99.73 & 98.95 & 73.60 & 93.30 \\
\bottomrule
\end{tabular}%
}
\end{table}
