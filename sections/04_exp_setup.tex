...
\begin{table}[t]
\caption{\textbf{Trained methods against ours on all seventeen benchmarks.} Recall@1, same splits, ground truth and configuration as Table~\ref{tab:breadth}. \emph{Dim} is the first-stage descriptor dimensionality, as evaluated. Baseline cells are published numbers, or our run of the released checkpoint where the paper omits the benchmark. $^{\dagger}$after the method's own re-ranker; $^{\ddag}$without the cross-image encoder. Bold $=$ best, \underline{underline} $=$ second, within each block.}
\label{tab:trained}
\centering
{\scriptsize\setlength{\tabcolsep}{2.0pt}\renewcommand{\arraystretch}{0.92}%
\begin{tabular}{ll ccccccccc|c}
