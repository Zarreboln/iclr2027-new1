...
\label{sec:prelim}
Given a query image $I_q$ and a database $\mathcal{D}=\{(I_d,\mathbf c_d)\}$ of images $I_d$ with known spatial coordinates $\mathbf c_d$, visual place recognition retrieves the database image that best matches the query and returns its coordinates as the estimate $\hat{\mathbf c}_q$ \citep{netvlad}. As discussed in \S\ref{}, most existing solutions train a dedicated model on a large annotated corpus $\mathcal{D}^{T}=\{(I_t,\mathbf c_t)\}$ for this retrieval, and the model has to be trained again whenever the backbone changes.
